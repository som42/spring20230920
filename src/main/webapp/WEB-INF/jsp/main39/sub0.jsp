  ...
  <script>
    function ajax2(){
      axios.get("/main39/sub2")
          // 응답 코드로 기준으로 한다 200이면 성공 나머지는 실패
              .then(() => console.log("성공"))
              .catch(() => console.log("실패")) // 실패시 실행되는 코드
    }
  </script>
</div>
<%--응답 코드 따라서 실패 메세지 보여주기 --%>
<div>
...
<div>
    ...
    <script>
        function ajax4(){
            ...
            axios.get("/main39/sub3") // GET요청을 보내는 함수
                .catch(function (error){ // 응답 받는 메소드를 사용하여 에러 처리
                    const  code = error.response.status; // 에러객체의 속성을 통해 상태코드확인
